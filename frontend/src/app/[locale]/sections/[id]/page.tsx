'use client';

import { useState } from 'react';
import { useParams, useSearchParams } from 'next/navigation';
import { useTranslations } from 'next-intl';
import { useSection } from '@/hooks/use-sections';
import { useProducts } from '@/hooks/use-products';
import type { ProductOrderByColumn, SortOrder } from '@/gql/graphql';
import { ProductGrid } from '@/components/ProductGrid';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Card, CardContent } from '@/components/ui/card';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { ArrowLeft, Grid, List, Sparkles, TrendingUp, Star, Clock } from 'lucide-react';
import Image from 'next/image';
import Link from 'next/link';
import { cn } from '@/lib/utils';
interface SortOption {
  value: string;
  label: string;
  orderBy: Array<{ column: ProductOrderByColumn; order: SortOrder }>;
}

const getSectionIcon = (sectionType: string) => {
  switch (sectionType) {
    case 'RECOMMENDATION':
      return <Sparkles className="w-5 h-5" />;
    case 'TRENDING':
      return <TrendingUp className="w-5 h-5" />;
    case 'NEW_ARRIVALS':
      return <Clock className="w-5 h-5" />;
    default:
      return <Star className="w-5 h-5" />;
  }
};

const getSectionColor = (sectionType: string) => {
  switch (sectionType) {
    case 'RECOMMENDATION':
      return 'bg-purple-500';
    case 'TRENDING':
      return 'bg-orange-500';
    case 'NEW_ARRIVALS':
      return 'bg-green-500';
    default:
      return 'bg-blue-500';
  }
};

export default function SectionPage() {
  const params = useParams();
  const searchParams = useSearchParams();
  const t = useTranslations();
  const sectionId = params.id as string;
  
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [gridColumns, setGridColumns] = useState<2 | 3 | 4 | 5>(4);
  const [sortBy, setSortBy] = useState(searchParams.get('sort') || 'featured');
  const [currentPage, setCurrentPage] = useState(1);
  const perPage = 24;

  // Use GraphQL hooks for data fetching
  const { section, loading: sectionLoading, error: sectionError } = useSection({ 
    id: sectionId 
  });

  const sortOptions: SortOption[] = [
    { 
      value: 'featured', 
      label: t('product.sort.featured'),
      orderBy: [{ column: 'IS_FEATURED', order: 'DESC' }, { column: 'CREATED_AT', order: 'DESC' }]
    },
    { 
      value: 'newest', 
      label: t('product.sort.newest'),
      orderBy: [{ column: 'CREATED_AT', order: 'DESC' }]
    },
    { 
      value: 'price_asc', 
      label: t('product.sort.priceLowToHigh'),
      orderBy: [{ column: 'PRICE', order: 'ASC' }]
    },
    { 
      value: 'price_desc', 
      label: t('product.sort.priceHighToLow'),
      orderBy: [{ column: 'PRICE', order: 'DESC' }]
    },
    { 
      value: 'name_asc', 
      label: t('product.sort.nameAtoZ'),
      orderBy: [{ column: 'NAME_EN', order: 'ASC' }]
    },
    { 
      value: 'name_desc', 
      label: t('product.sort.nameZtoA'),
      orderBy: [{ column: 'NAME_EN', order: 'DESC' }]
    },
  ];

  const currentSortOption = sortOptions.find(option => option.value === sortBy) || sortOptions[0];

  // Get products for this section using section products relation
  // For now, use section.products from the section query
  // If more advanced filtering is needed, a separate products query can be added
  const sectionProducts = section?.products || [];
  
  // Apply client-side sorting if needed (GraphQL doesn't support sorting section products)
  const sortedProducts = [...sectionProducts].sort((a, b) => {
    const sortOption = currentSortOption.orderBy[0];
    switch (sortOption.column) {
      case 'PRICE':
        return sortOption.order === 'ASC' ? a.price - b.price : b.price - a.price;
      case 'CREATED_AT':
        // Assuming newer products have higher IDs for simplicity
        return sortOption.order === 'ASC' ? 
          parseInt(a.id) - parseInt(b.id) : 
          parseInt(b.id) - parseInt(a.id);
      case 'IS_FEATURED':
        if (a.is_featured === b.is_featured) {
          return parseInt(b.id) - parseInt(a.id); // Then by newest
        }
        return sortOption.order === 'ASC' ? 
          (a.is_featured ? 1 : 0) - (b.is_featured ? 1 : 0) :
          (b.is_featured ? 1 : 0) - (a.is_featured ? 1 : 0);
      case 'NAME_EN':
        return sortOption.order === 'ASC' ? 
          a.name.localeCompare(b.name) : 
          b.name.localeCompare(a.name);
      default:
        return 0;
    }
  });

  const handleSortChange = (value: string) => {
    setSortBy(value);
  };

  if (sectionError) {
    console.error('Section query error:', sectionError);
  }

  const isLoading = sectionLoading;

  const getSectionDescription = (sectionType: string) => {
    switch (sectionType) {
      case 'RECOMMENDATION':
        return t('section.recommendationDescription');
      case 'TRENDING':
        return t('section.trendingDescription');
      case 'NEW_ARRIVALS':
        return t('section.newArrivalsDescription');
      default:
        return t('section.defaultDescription');
    }
  };

  if (isLoading) {
    return (
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="animate-pulse">
          <div className="h-8 bg-gray-200 rounded w-1/4 mb-8"></div>
          <div className="h-32 bg-gray-200 rounded-lg mb-8"></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {Array.from({ length: 8 }).map((_, index) => (
              <div key={index} className="space-y-3">
                <div className="aspect-[4/3] bg-gray-200 rounded"></div>
                <div className="h-4 bg-gray-200 rounded w-3/4"></div>
                <div className="h-4 bg-gray-200 rounded w-1/2"></div>
              </div>
            ))}
          </div>
        </div>
      </div>
    );
  }

  if (!section) {
    return (
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="text-center py-16">
          <h1 className="text-2xl font-semibold text-gray-900 mb-4">
            {t('section.notFound')}
          </h1>
          <p className="text-gray-600 mb-8">
            {t('section.notFoundDescription')}
          </p>
          <Link href="/products">
            <Button>
              <ArrowLeft className="w-4 h-4 mr-2" />
              {t('product.backToProducts')}
            </Button>
          </Link>
        </div>
      </div>
    );
  }

  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      {/* Breadcrumb */}
      <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-8">
        <Link href="/" className="hover:text-primary">
          {t('common.home')}
        </Link>
        <span>/</span>
        <Link href="/products" className="hover:text-primary">
          {t('product.products')}
        </Link>
        <span>/</span>
        <span className="text-gray-900">{section.title}</span>
      </nav>

      {/* Section Header */}
      <div className="mb-8">
        <Card className="relative overflow-hidden">
          <div className={cn(
            "absolute inset-0 opacity-10",
            getSectionColor(section.section_type)
          )} />
          <CardContent className="relative p-8">
            <div className="flex items-center gap-4 mb-4">
              <div className={cn(
                "w-12 h-12 rounded-full flex items-center justify-center text-white",
                getSectionColor(section.section_type)
              )}>
                {getSectionIcon(section.section_type)}
              </div>
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">
                  {section.title}
                </h1>
                <p className="text-gray-600 text-lg">
                  {getSectionDescription(section.section_type)}
                </p>
              </div>
            </div>
            
            <div className="flex items-center gap-4">
              <Badge 
                variant="secondary" 
                className={cn(
                  "text-white",
                  getSectionColor(section.section_type)
                )}
              >
                {section.active_products_count} {t('section.products')}
              </Badge>
              
              <Badge variant="outline">
                {t(`section.type.${section.section_type.toLowerCase()}`)}
              </Badge>
            </div>
          </CardContent>
        </Card>
      </div>

      {/* Controls Bar */}
      <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          <h2 className="text-lg font-semibold text-gray-900">
            {t('section.allProducts')} ({section.active_products_count})
          </h2>
        </div>

        <div className="flex items-center gap-4">
          {/* Sort */}
          <Select value={sortBy} onValueChange={setSortBy}>
            <SelectTrigger className="w-48">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {sortOptions.map((option) => (
                <SelectItem key={option.value} value={option.value}>
                  {option.label}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>

          {/* View mode toggle */}
          <div className="flex items-center border rounded-lg p-1">
            <Button
              variant={viewMode === 'grid' ? 'default' : 'ghost'}
              size="sm"
              onClick={() => setViewMode('grid')}
              className="px-2"
            >
              <Grid className="w-4 h-4" />
            </Button>
            <Button
              variant={viewMode === 'list' ? 'default' : 'ghost'}
              size="sm"
              onClick={() => setViewMode('list')}
              className="px-2"
            >
              <List className="w-4 h-4" />
            </Button>
          </div>

          {/* Grid columns (only for grid view) */}
          {viewMode === 'grid' && (
            <Select value={gridColumns.toString()} onValueChange={(value: string) => setGridColumns(Number(value) as 2 | 3 | 4 | 5)}>
              <SelectTrigger className="w-20">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="2">2</SelectItem>
                <SelectItem value="3">3</SelectItem>
                <SelectItem value="4">4</SelectItem>
                <SelectItem value="5">5</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
      </div>

      {/* Products Grid */}
      <ProductGrid
        products={sortedProducts.map(product => ({
          ...product,
          discount_percentage: product.discount_percentage ?? undefined,
          featured_image: product.featured_image ?? undefined,
          description: product.description ?? undefined,
        }))}
        isLoading={false}
        columns={gridColumns}
        variant={viewMode === 'list' ? 'compact' : 'default'}
      />

      {/* Section Info */}
      {sortedProducts.length === 0 && !isLoading && (
        <div className="text-center py-16">
          <div className={cn(
            "w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center text-white",
            getSectionColor(section.section_type)
          )}>
            {getSectionIcon(section.section_type)}
          </div>
          <h3 className="text-xl font-semibold text-gray-900 mb-2">
            {t('section.noProducts')}
          </h3>
          <p className="text-gray-500 max-w-md mx-auto">
            {t('section.noProductsDescription')}
          </p>
        </div>
      )}
    </div>
  );
}